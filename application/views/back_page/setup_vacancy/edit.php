<div class="main-content">
    ...
    <form role="form" method="post" class="form-horizontal form-groups-bordered validate" action="<?php echo site_url('back/SetupVacancy/updateVacancy');?>">
        <?php foreach ($_getVacancy as $row): ?>
        <div class="form-group" >
            <label for="inputTitlePost" class="col-sm-1 control-label">Title Post</label>
            <div class="col-sm-8">
                <input type="hidden" class="form-control" id="inputIdVacancy" name="txtIdVacancy" value="<?php echo $row->id_vacancy;?>" >
                <input type="text" class="form-control" id="inputTitlePost" name="txtTitlePost" placeholder="Input Title" data-validate="required"
                       data-message-required="Please insert this field!" value="<?php echo $row->title_jobs;?>">
            </div>
        </div>
        ...
        <div class="form-group" >
            <div class="col-sm-9">
                <textarea id="inputPostValue" name="txtPostValue" class="form-control ckeditor"><?php echo $row->description;?></textarea>
            </div>

            <div class="col-sm-3">
                <label for="inputCategory" class="control-label">Closing Date</label>
                <div class="input-group">
                    <input type="text" class="form-control datepicker" name="txtDateClosing" data-format="dd-mm-yyyy" data-mask="d-m-y"
                           placeholder="Input Date Closing.." value="<?php echo date('d-m-Y', strtotime($row->closed_date));?>">
                ...
                <select id="inputCategory" name="selCategory" class="form-control select" >
                    ...
                    foreach ($_selectCategory as $rowCat):
                        if($rowCat->id_category == $row->id_category):
                            echo "<option value='$rowCat->id_category' selected>$rowCat->name_category</option>";
                        else:

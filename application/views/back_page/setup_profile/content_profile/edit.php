...
<?php foreach ($_selectContent as $row):?>
    <form role="form" method="post" class="form-horizontal form-groups-bordered validate" action="<?php echo site_url('back/setupProfile/updateContentProfile'); ?>" enctype="multipart/form-data" >
        <div class="form-group" >
            ...
            <div class="col-sm-8">
                <input type="text" class="form-control" id="inputTitlePost" name="txtTitlePost" placeholder="Input Title" data-validate="required"
                       data-message-required="Please insert this field!" value="<?php echo $row->title_profile;?>">
                <input type="hidden" class="form-control" id="inputKodeProfile" name="txtKodeProfile" value="<?php echo $row->kode_profile;?>">
            </div>
        </div>
        ...
        <div class="form-group" >
            <div class="col-sm-9">
                <textarea id="inputPostValue" name="txtPostValue" class="form-control ckeditor"><?php echo $row->post_profile;?></textarea>
            </div>
            ...
            <div class="col-sm-3">
                ...
                <select id="inputHeader" name="selHeader" class="form-control select" >
                    ...
                    foreach ($_selectHead as $rowHead):
                        if($row->id_head == $rowHead->id_head_profile){
                            echo "<option value='$rowHead->id_head_profile' selected>$rowHead->head_descript</option>";
                        }else{
                ...
                <br/>
                <button class="btn btn-block btn-primary" type="submit"><i class="fa fa-edit"></i>&nbsp; Update</button>
            </div>
        </div>
        ...
        <div class="form-group">            
            ...
            <div class="col-sm-1">
                <input class="icheck-2" type="radio" id="radioLayout1" name="radioLayout" value="1" onchange="selectLayout(this.value)" <?php if($row->layout_content == 1){ echo 'checked';}?> >
                <label for="radioLayout1">Full Text</label>
                <img src="<?php echo base_url('assets/img/layout-3.png');?>" class="img-responsive" >
            </div>
            <div class="col-sm-1">
                <input class="icheck-2" type="radio" id="radioLayout2" name="radioLayout" value="2" onchange="selectLayout(this.value)" <?php if($row->layout_content == 2){ echo 'checked';}?> >
                <label for="radioLayout2">On Top</label>
                <img src="<?php echo base_url('assets/img/layout-1.png');?>" class="img-responsive" >
            </div>
            <div class="col-sm-1">
                <input class="icheck-2" type="radio" id="radioLayout3" name="radioLayout" value="3" onchange="selectLayout(this.value)" <?php if($row->layout_content == 3){ echo 'checked';}?> >
                <label for="radioLayout3">On Right</label>
                <div class="col-sm-1"></div>
                <img src="<?php echo base_url('assets/img/layout-2.png');?>" class="img-responsive" >
            </div>
            <?php
                if($row->layout_content == 1){
                    $display    = 'none';
                }else{
                    $display    = 'block';
                }
            ?>
            <div id="inputImageHeader" style="display: <?php echo $display;?>;">
                <label for="inputTitlePost" class="col-sm-1 control-label">Image Header</label>
                <div class="col-sm-2">
                    <!-- <input type="file" name="imgHeader" id="imgHeader" accept="image/*"> -->
                    <div class="fileinput fileinput-new" data-provides="fileinput">
                        <div class="fileinput-new thumbnail" style="width: 100%; height: 200px" data-trigger="fileinput">
                            <img src="/SambuPage/assets/img/thumb.png" alt="..." class="img-rounded">
                        </div>
